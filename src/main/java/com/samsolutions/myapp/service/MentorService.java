...
public class MentorService {
	...
	public ModelAndView getMentorPage(String page){
		...
		List<Long> usersId = contacts.stream().map(c-> c.getUser().getId()).collect(Collectors.toList());
		List<Result> results = new ArrayList<>();
		for (long id: usersId
		) {
			results.addAll(resultDAOService.getResultsByUserIdAndGrade(id,0));
		}
		List<Long> resultsId = results.stream().map(c->c.getId()).collect(Collectors.toList());
